...
function singleProductPrint() {
  ...
  if (productData) {
    // il evaluam                                                          // ii dam un count setat la 1
    ...
    starsHtml += `<p>${productData.rating.rate}</p>`;
            ...
            <div class="size-color">
              ...
              <div class="butoane-size">
                ...
                <label>
                  ...
                  <span class="size-button">XS</span>
                </label>
              </div>
              <h5>Color :</h5> 
              <div class="butoane-color">
                <label>
                  <input type="radio" name="color" value="color1">
                  <span class="color-swatch" style="background-color: rgba(111, 155, 176);"></span>
                </label>
                <label>
                ...
                <label>
                  ...
                  <span class="color-swatch" style="background-color: #b88e2f;"></span>
                </label>
              </div>
            </div>
            <div class="controale"> 
    ...
    buttonPlusSP.addEventListener("click", function () {
      // functie pt a creste countul de la inceput
      count++;
    ...
    buttonMinusSP.addEventListener("click", function () {
      // lafel ca mai sus doar ca in scadere si extra o conditie care spune ca tre sa fie mai mare decat 1 pt a nu scadea sub 1
      if (count > 1) {
        count--;
    ...
    butonAdaugare.addEventListener("click", function () {
      // functie setata pe buton pt a apela alta functie si a adauga in cos la final
...
function adaugaInCosSP(produs) {
  // fata de adauga in cos clasic aceasta functie are in vedere si countul produsului deoarece adauga in cos clasic este din interfata home/shop unde countul nu e setat inca
  ...
  if (index > -1) {
    // daca exista i se adauga countul existent la cel setat de butoanele -+
    cosDinLocalStorage[index].count =
      cosDinLocalStorage[index].count + produs.count;
  } else {
    // in caz ca nu exista este inpins in LS impreuna cu un count setat in functia de printare
...
function updateBreadcrumb() {
  // functie ce are grija sa verifice de titlu si sa il afiseze sub nav pt un UI mai interactiv
  ...
  if (productData && productData.title) {
    ...
    // in caz ca lipseste va inlocui titlul din breadcrumb cu un scenariu general
    document.querySelector(
      ".Product-breadcrumb"
